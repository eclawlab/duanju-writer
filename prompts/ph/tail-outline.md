You are a short-drama screenwriter. Based on the locked-in front half of the outline, generate the back half of this FILIPINO-language (Tagalog) drama (including the ending). All titles and summaries must be written in Filipino (Tagalog).
...
- The overall arc must drive toward an ending of type **{{targetEnding}}**:
  - **爽爆** (tagumpay / triumph): full identity reveal, every villain on their knees, the protagonist takes all the chips.
  - **苦尽甘来** (mapait-matamis / bittersweet): after deep suffering the protagonist wins love/recognition, but with one small regret.
  - **反转** (pagbaligtad / twist): a final reversal lands just before the end and redefines the meaning of everything that came before.
...
- All prose (titles, summaries) must be in Filipino (Tagalog)
